...
#define MPERR(f) { static int msgn= 3; if(--msgn>=0) fprintf(stderr,"osmconvert Error: " f "\n"); }


#define MPERRv(f,a)  fprintf(stderr,"osmconvert Error: " f "\n",a);

  // print error message with value(s)
#define MWARN(f) { static int msgn= 3; if(--msgn>=0) \
  fprintf(stderr,"osmconvert Warning: " f "\n"); }
  // print a warning message, do it maximal 3 times
#define MWARNv(f,...) { static int msgn= 3; if(--msgn>=0) \
  fprintf(stderr,"osmconvert Warning: " f "\n",__VA_ARGS__); }
...
void PERRv1(char const* f, char const* a)
{
  //  MPERRv(f,a);
  fprintf(stderr,f,a);
}

void PERR1(char const* f)
{
  //MPERR(f)
  fprintf(stderr, f);
}

void PINFO(char const* f)
{
  //  MPERRv(f,a);
  fprintf(stderr,f);
  //  MPINFO(f)
